...
-- Nome do aluno: João Vitor Veglione
...
SELECT
	...
	DescricaoProduto as "Descrição do produto",
	UnidadeProduto as "Unidade de medida",
	ValorUnitario as "Valor unitario"
	
	FROM PRODUTOS
    WHERE ValorUnitario = (SELECT MAX(ValorUnitario) from produtos) or
	ValorUnitario = (SELECT MIN(ValorUnitario) from produtos)
...
INNER JOIN PEDIDOS AS PED
    ON C.CodigoCliente = PED.CodigoCliente
INNER JOIN ITEM_DE_PEDIDO AS I
    ON PED.NumeroPedido = I.NumeroPedido
INNER JOIN PRODUTOS AS P
    ON I.CodigoProduto = P.CodigoProduto
INNER JOIN VENDEDORES AS V
    ON PED.CodigoVendedor = V.CodigoVendedor
ORDER BY
    C.NomeCliente,
    P.DescricaoProduto,
    I.Quantidade,
